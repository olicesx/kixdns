...
// ============================================================================
// Re-exports & shared server logic
// ============================================================================
...
// ============================================================================
// run_dns_server  —  console mode (no external shutdown signal)
// ============================================================================

/// 运行 DNS 服务器（控制台模式）/ Run DNS server (console mode)
pub async fn run_dns_server(
    ...
    let bind_tcp: SocketAddr = cfg.settings.bind_tcp.parse().context("parse tcp bind addr")?;
    ...
    let needs_ipv4_tcp = bind_tcp.is_ipv4()
        || (bind_tcp.is_ipv6() && bind_tcp.ip().is_unspecified());
...
// ============================================================================
// run_dns_server_with_shutdown  —  service mode (can be stopped via oneshot)
// ============================================================================

/// 运行 DNS 服务器并支持通过 oneshot 通道优雅关闭
/// Run DNS server with graceful shutdown via a oneshot channel
///
/// This function is called by the service module when running as a Windows
/// service. The `shutdown_rx` receiver is signalled when SCM requests stop.
/// 此函数在作为 Windows 服务运行时由 service 模块调用。
/// 当 SCM 请求停止时，`shutdown_rx` 接收器会收到信号。
pub async fn run_dns_server_with_shutdown(
    config: PathBuf,
    listener_label: String,
    shutdown_rx: oneshot::Receiver<()>,
) -> anyhow::Result<()> {
    // Init tracing — no debug flag, use env var or default to info
    // 初始化 tracing —— 无 debug 标志，使用环境变量或默认 info 级别
    init_tracing(false);
    ...
    let udp_workers_final = num_cpus::get().max(1);

    info!(
        bind_udp = %bind_addr,
        bind_tcp = %bind_tcp,
        udp_workers_count = udp_workers_final,
        "dns server started (service mode)"
    );
    ...
    tokio::select! {
        _ = shutdown_rx => {
            info!("Service shutdown signal received, stopping server...");
            // Tasks will be dropped when we return, so no need to explicitly cancel
            // 返回时任务会被自动丢弃，无需显式取消
        }
        _ = async {
            ...
            info!("All server tasks completed (unexpected)");
        }
    }
...
// ============================================================================
// UDP worker spawning (Unix — SO_REUSEPORT)
// ============================================================================
    ...
    loop {
        ...
        let resp = match engine.handle_packet_fast(&packet_bytes, peer) {
            ...
            Ok(Some(FastPathResponse::CacheHit {
                ...
                let elapsed = inserted_at.elapsed().as_secs() as u32;
                if elapsed > 0 {
                    crate::proto_utils::patch_all_ttls(&mut resp_buf, elapsed);
                }
