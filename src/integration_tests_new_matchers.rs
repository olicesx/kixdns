...
mod integration_tests {
    ...
    fn test_geoip_matcher_integration() {
        ...
        let public_ips = vec!["8.8.8.8", "1.1.1.1", "2606:4700:4700::1111"];
    ...
    fn test_new_matchers_config_parsing() {
        ...
        assert_eq!(
            rule1.name.as_ref(),
            "geosite_test",
            "First rule name should match"
        );
        assert_eq!(
            rule1.matchers.len(),
            2,
            "First rule should have two matchers"
        );
        ...
        assert_eq!(
            rule2.name.as_ref(),
            "geoip_test",
            "Second rule name should match"
        );
        assert_eq!(
            rule2.matchers.len(),
            2,
            "Second rule should have two matchers"
        );
        ...
        assert_eq!(
            rule3.name.as_ref(),
            "qtype_test",
            "Third rule name should match"
        );
        assert_eq!(
            rule3.matchers.len(),
            1,
            "Third rule should have one matcher"
        );
    ...
    fn test_pipeline_selector_with_new_matchers() {
        ...
        assert_eq!(
            runtime.pipeline_select.len(),
            2,
            "Should have exactly two pipeline selectors"
        );

        // Assert: Verify GeoSite in pipeline selector
        let sel1 = &runtime.pipeline_select[0];
        assert_eq!(
            sel1.pipeline, "china_pipe",
            "First selector should target china_pipe"
        );
        assert_eq!(
            sel1.matchers.len(),
            1,
            "First selector should have one matcher"
        );

        // Assert: Verify Qtype in pipeline selector
        let sel2 = &runtime.pipeline_select[1];
        assert_eq!(
            sel2.pipeline, "ipv6_pipe",
            "Second selector should target ipv6_pipe"
        );
        assert_eq!(
            sel2.matchers.len(),
            1,
            "Second selector should have one matcher"
        );
    ...
    fn test_edge_cases_and_error_handling() {
        ...
        assert!(
            !empty_geosite.matches("cn", "www.baidu.com"),
            "Empty GeoSite database should not match any domain"
        );
        assert!(
            !empty_geosite.matches("google", "www.google.com"),
            "Empty GeoSite database should not match any domain"
        );
        ...
        assert!(
            !result.is_private,
            "0.0.0.0 should not be in private ranges"
        );
    ...
    fn test_performance_and_caching() {
        ...
        assert!(
            geosite_mgr.matches("test", "www.test.com"),
        ...
        assert!(
            result1.is_private,
            "192.168.1.1 should be identified as private IP"
        );
        ...
        assert!(
            result2.is_private,
            "Cached lookup should also identify as private IP"
        );
